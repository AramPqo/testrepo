...
/**
 * REST controller for managing {@link com.novanus.medappoint.domain.Business}.
 */
@RestController
@RequestMapping("/api")
public class BusinessResource {
    ...
    /**
     * {@code GET  /businesses/:id/cus-col} : get the Business Customer_colors.
     *
     * @param id the id of the businessDTO to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the businessDTO, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/businesses/{id}/cus-col")
    public ResponseEntity<String> getBusinessCustomColors(@PathVariable Long id) {
        log.debug("REST request to get Business Custom_colors : {}", id);
        Optional<BusinessDTO> businessDTO = businessService.findOne(id);
        String customColors = businessDTO.get().getCustomColors();

        return ResponseEntity.ok(customColors);
    }

    /**
     * {@code PUT  /cus-col} : Updates an existing business.
     *
     * @param businessDTO the businessDTO Custom_colors to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated businessDTO Custom_colors,
     * or with status {@code 400 (Bad Request)} if the businessDTO Custom_colors is not valid,
     * or with status {@code 500 (Internal Server Error)} if the businessDTO Custom_colors couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/businesses/cus-col")
    public ResponseEntity<BusinessDTO> updateBusinessCustomColors(@RequestBody BusinessDTO businessDTO) throws URISyntaxException {
        log.debug("REST request to update Business Custom_colors : {}", businessDTO);
        if (businessDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }

        BusinessDTO result = businessService.updateColor(businessDTO);
        return ResponseEntity.ok()
                .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, businessDTO.getId().toString()))
                .body(result);
    }
