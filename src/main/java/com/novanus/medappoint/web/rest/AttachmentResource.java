...
/**
 * REST controller for managing {@link com.novanus.medappoint.domain.Attachment}.
 */
@RestController
@RequestMapping("/api")
public class AttachmentResource {
    ...
    public AttachmentResource(AttachmentService attachmentService, AttachmentFileService attachmentFileService) {
    ...
    /**
     * {@code GET  /attachments/:id/download} : get the "id" attachment.
     *
     * @param id the id of the attachmentDTO to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the attachment binary, or with status {@code 404 (Not Found)}.
     */
    ...
    public ResponseEntity<byte[]> downloadAttachment(@PathVariable Long id) {
        ...
        if (attachmentDTOOptional.isPresent()) {
            ...
            Optional<AttachmentFileDTO> attachmentFileDTOOptional = attachmentFileService.findOne(attachmentDTO.getAttachmentFileId());
            if (attachmentFileDTOOptional.isPresent()) {
                AttachmentFileDTO attachmentFileDTO = attachmentFileDTOOptional.get();
                HttpHeaders headers = new HttpHeaders();
                headers.setContentType(MediaType.valueOf(attachmentFileDTO.getFileContentType()));

                headers.setContentDispositionFormData(attachmentDTO.getFileName(), attachmentDTO.getFileName());
                headers.setCacheControl("must-revalidate, post-check=0, pre-check=0");
                byte[] contents = attachmentFileDTO.getFile();
                response = new ResponseEntity<>(contents, headers, HttpStatus.OK);
            } else {
                response = new ResponseEntity("Cannot access file " + attachmentDTO.getFileName(), HttpStatus.INTERNAL_SERVER_ERROR);
            }
        } else {
            response = new ResponseEntity("Cannot access attachment", HttpStatus.INTERNAL_SERVER_ERROR);
        }
        return response;
    }
}
